...
describe('AuthService - Registration Methods', () => {
  ...
  describe('registerUser', () => {
    beforeEach(() => {
      ...
      mockDatabaseService.query
        ...
        .mockResolvedValueOnce({ rows: [{ ...mockUser, first_name: 'John', last_name: 'Doe', is_active: true, email_verified: false }] }); // Create user
    ...
    it('should register user successfully with email verification', async () => {
      ...
      expect(emailService.sendVerificationEmail).toHaveBeenCalledWith(
        expect.objectContaining({
          ...
          verificationUrl: expect.stringContaining('/auth/verify-email/verification-token-123')
        })
      );
    ...
    it('should handle existing user email', async () => {
      // Arrange
      mockDatabaseService.query
        .mockResolvedValueOnce({ rows: [{ id: 'existing-user', email_verified: true }] }); // Existing user found
    ...
    it('should validate email format and domain restrictions', async () => {
      ...
      const restrictedEmailData = { ...mockRegistrationData, email: 'user@external.com' };
      
      await expect(AuthService.registerUser(restrictedEmailData))
        .rejects.toThrow('Email validation failed');
    });
    ...
    it('should validate names for registration', async () => {
      ...
      const suspiciousNameData = { ...mockRegistrationData, firstName: 'test', lastName: 'user' };
      
      await expect(AuthService.registerUser(suspiciousNameData))
        .rejects.toThrow('Name validation failed');
    });

    it('should handle admin approval requirement', async () => {
      // Arrange
      mockEmailVerificationService.getRegistrationSettings.mockResolvedValue({
        id: 'settings-1',
    ...
    it('should log registration audit events', async () => {
      // Mock the logRegistrationEvent method
      const logRegistrationEventSpy = jest.spyOn(AuthService as any, 'logRegistrationEvent')
      ...
      // Assert
      expect(logRegistrationEventSpy).toHaveBeenCalledWith(
        expect.objectContaining({
          userId: mockUser.id,
          ...
          eventData: expect.objectContaining({
            email: mockUser.email,
            role: mockUser.role,
            passwordStrength: expect.any(String)
          }),
          ipAddress: mockRegistrationData.ipAddress,
          userAgent: mockRegistrationData.userAgent
        })
      );

      logRegistrationEventSpy.mockRestore();
    });

    it('should detect and log suspicious registration patterns', async () => {
      // Mock the checkSuspiciousRegistration method
      const checkSuspiciousSpy = jest.spyOn(AuthService as any, 'checkSuspiciousRegistration')
        .mockResolvedValue({
          isSuspicious: true,
          reasons: ['Multiple registrations from same IP', 'Unusual user agent pattern']
        });
      ...
      // Assert
      expect(checkSuspiciousSpy).toHaveBeenCalledWith(
        mockRegistrationData.email,
        mockRegistrationData.ipAddress,
        mockRegistrationData.userAgent
      );
      ...
      expect(mockLogger.warn).toHaveBeenCalledWith(
        ...
        expect.objectContaining({
          email: mockRegistrationData.email.toLowerCase(),
          ipAddress: mockRegistrationData.ipAddress,
          reasons: expect.arrayContaining(['Multiple registrations from same IP'])
        })
      );

      checkSuspiciousSpy.mockRestore();
    });
    ...
    it('should handle database errors during user creation', async () => {
      ...
      mockDatabaseService.query
        .mockResolvedValueOnce({ rows: [] }) // Check existing user succeeds
        .mockRejectedValueOnce(dbError); // Create user fails
    ...
    it('should use custom verification token expiry from settings', async () => {
      // Arrange
      ...
      // Assert
      const emailService = (AuthService as any).emailService;
      const emailCall = emailService.sendVerificationEmail.mock.calls[0][0];
      
      // Verify expiry time is approximately 48 hours from now
      const expectedExpiry = new Date(Date.now() + customExpiryHours * 60 * 60 * 1000);
      const actualExpiry = emailCall.expiresAt;
      
      expect(Math.abs(actualExpiry.getTime() - expectedExpiry.getTime())).toBeLessThan(60000); // Within 1 minute
    });
  ...
  describe('validateRegistrationPassword', () => {
    ...
    it('should enforce minimum score requirement', () => {
      // Test password that meets character requirements but is still weak
      const result = AuthService.validateRegistrationPassword('Aa1!');
      expect(result.isValid).toBe(false);
    ...
    it('should calculate strength levels correctly', () => {
      const testCases = [
        ...
        { password: 'Fair1Pass!', expectedStrength: 'fair' },
        { password: 'Good1Password!', expectedStrength: 'good' },
        { password: 'Str0ng&C0mpl3x!P@ssw0rd', expectedStrength: 'strong' }
      ];
  ...
  describe('validateRegistrationEmail', () => {
    ...
    it('should handle settings retrieval errors gracefully', async () => {
      // Arrange
      mockEmailVerificationService.getRegistrationSettings.mockRejectedValue(
        new Error('Settings service unavailable')
      );

      // Act
      const result = await AuthService.validateRegistrationEmail('user@example.com');

      // Assert - Should still validate basic format
      expect(result.isValid).toBe(true);
      expect(mockLogger.error).toHaveBeenCalledWith(
        'Error validating registration email',
        expect.objectContaining({
